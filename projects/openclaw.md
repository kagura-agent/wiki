...
- **#110602** (2026-07-18, PENDING): fix(agents): bypass messaging tool suppression for synthetic media-only payloads. Fixes #110394. Root cause: `mergeAttemptToolMediaPayloads` creates synthetic media-only payload when tools produce media but no visible text; this payload gets blanket-dropped by `suppressMessagingToolReplies`. Fix: added `bypassMessagingToolSuppression` flag on synthetic payload + filter logic in `buildReplyPayloads`. CI: all checks pass. Tests: 24/24 (3 tool-media + 21 agent-runner-payloads).
