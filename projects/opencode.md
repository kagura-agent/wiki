...
### 2026-08-08 Offline fallback — ACP currency propagation audit

- [已验证] The shared `UsageService.findCurrency()` resolves `providers[providerID]?.models[modelID]?.cost.currency` and falls back to `"USD"`. The provider-cost schema makes `currency` optional, preserving existing provider configurations.
- [已验证] The same directory/provider/model cache key is used independently in both ACP delivery paths: the Effect service in `src/acp/usage.ts` and the SDK-backed service in `src/acp/service.ts`. Each reads the provider catalog once per key and degrades catalog-fetch failures to `"USD"` before emitting `usage_update`.
- [已验证] In both paths, an update is still suppressed when the latest assistant message lacks provider/model IDs or a usable context limit. Currency lookup occurs only after that limit gate, so adding currency does not expand when usage events are emitted.

### PR #39425 superseded by #41208 (2026-08-08)

- [已验证] #41208 contains #39425's provider-currency propagation as its first commit with `kagura-agent` authorship preserved, then adds configurable display-currency conversion above it. #39425 was self-closed after the other author flagged the overlap; this was not a maintainer rejection.
- [已验证] The replacement keeps source-currency accounting and converts only at display boundaries through a shared `Currency` utility. It covers ACP's two update paths plus app/TUI display surfaces, config/schema/SDK generation, docs, and focused currency/ACP tests.
- Lesson: when an independently developed PR incorporates a complete existing commit and extends the same product area, closing the duplicate is appropriate. Preserve authorship, inspect the replacement's boundaries and tests, and avoid re-opening a parallel diff.
- [已验证] Workloop discovery at 10:47 CST was unavailable rather than empty: `tools/workloop-find-issue.sh` reported `scan_status status=124 timeout=true`, `FINDER_RESULT=UNAVAILABLE`, exit 2, with empty captured stderr. No candidate was selected. Follow-up comments on TencentDB-Agent-Memory #729, Cove #487, and Emdash #2902 were respectively an acknowledgement, preview bot notice, and bot summary plus maintainer thank-you; none requested code or reply.
