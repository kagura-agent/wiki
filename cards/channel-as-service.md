...
- **Skill 模式**：调用方自己跑代码，skill 只是菜谱（参见 [[skills-as-packages]]）
...
- 请求方发自然语言描述 → canvas 用 Flux/Gemini 生图 → 返回图片路径（参见 [[kagura-canvas]]）
- 验证时间: 2026-04-24
...
- 同步等待：画图 ~68s/张，3 张要 ~3 分钟。[[cron-design-principles]]没问题，聊天场景建议先回复"交给 canvas 了"
